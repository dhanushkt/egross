<div class="left-sidenav">

    <ul class="metismenu left-sidenav-menu" id="side-nav">


        <li>
            <a href="index.php"><i class="mdi mdi-view-dashboard-outline"></i><span>Dashboard</span></a>
        </li>

        <li>
            <a href="javascript: void(0);"><i class="mdi mdi-store"></i><span>Shops</span><span class="menu-arrow"><i class="mdi mdi-chevron-right"></i></span></a>
            <ul class="nav-second-level" aria-expanded="false">
                <li><a href="view-shops.php"><span>View Shops</span></a></li>
            </ul>
        </li>

        <li>
            <a href="javascript: void(0);"><i class="mdi mdi-sitemap"></i><span>Category</span><span class="menu-arrow"><i class="mdi mdi-chevron-right"></i></span></a>
            <ul class="nav-second-level" aria-expanded="false">
                <li>
                    <a href="javascript: void(0);">Main Category <span class="menu-arrow left-has-menu"><i class="mdi mdi-chevron-right"></i></span></a>
                    <ul class="nav-second-level" aria-expanded="false">
                        <li><a href="add-maincategory.php">Add</a></li>
                        <li><a href="view-maincategory.php">View</a></li>
                    </ul>
                </li>
                <li>
                    <a href="javascript: void(0);">Sub Category <span class="menu-arrow left-has-menu"><i class="mdi mdi-chevron-right"></i></span></a>
                    <ul class="nav-second-level" aria-expanded="false">
                        <li><a href="add-subcategory.php">Add</a></li>
                        <li><a href="view-subcategory.php">View</a></li>
                    </ul>
                </li>
            </ul>
        </li>

        <!-- <li>
            <a href="javascript: void(0);"><i class="mdi mdi-basket"></i><span>Items</span><span class="menu-arrow"><i class="mdi mdi-chevron-right"></i></span></a>
            <ul class="nav-second-level" aria-expanded="false">
                <li><a href="ui-alerts.html">View Items</a></li>
            </ul>
        </li> -->

        <li>
            <a href="javascript: void(0);"><i class="mdi mdi-account-group"></i><span>Users</span><span class="menu-arrow"><i class="mdi mdi-chevron-right"></i></span></a>
            <ul class="nav-second-level" aria-expanded="false">
                <li><a href="view-users.php">View Users</a></li>
            </ul>
        </li>

    </ul>
</div>